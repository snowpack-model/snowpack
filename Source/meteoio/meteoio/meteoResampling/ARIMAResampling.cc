...
{
	const auto it = std::lower_bound(vecMet.begin(), vecMet.end(), resampling_date, [](const MeteoData &a, const Date &b) { return a.date < b; });
...
{
	const auto exactTime = [&resampling_date](Date curr_date) { return requal(curr_date, resampling_date); };
...
{
	const auto exactTime = [&resampling_date](MeteoData curr_date) { return requal(curr_date.date, resampling_date); };
...
bool ARIMAResampling::processKnownGaps(const Date &resampling_date, const size_t paramindex, const ResamplingAlgorithms::ResamplingPosition &position, const std::vector<MeteoData> &vecM, MeteoData &md) 
{
	...
	for (size_t ii = 0; ii < gap_data.size(); ii++) {
		...
		if (resampling_date >= gap.startDate && resampling_date <= gap.endDate) {
			...
			if (verbose) std::cout << "Process a known gap" << std::endl;
			
			auto it = findDate(gap_dates, resampling_date);
			// if there is an exact match, return the data
			if (it != gap_dates.end()) {
				const size_t idx = std::distance(gap_dates.begin(), it);
				double value = data_in_gap[idx];
				md(paramindex) = value; // propagate value
				return true;
			} else {
				// otherwise linearly interpolate the data
				const size_t idx = std::distance(gap_dates.begin(), std::lower_bound(gap_dates.begin(), gap_dates.end(), resampling_date));
												...
												bool has_data_after, size_t paramindex, std::vector<double> &data, std::vector<Date> &dates, size_t length) 
{
	...
	for (size_t i = 0; i < length; i++) {
		...
		const bool isBeforeGap = date >= data_start_date && date <= new_gap.startDate;
		const bool isAfterGap = date >= new_gap.endDate && date <= data_end_date;
		const bool isWithinGap = date > new_gap.startDate && date < new_gap.endDate;
...
// ------------------------------ Resample ------------------------------
bool ARIMAResampling::resample(const std::string & /*stationHash*/, const size_t &index, const ResamplingPosition &position, const size_t &paramindex, const std::vector<MeteoData> &vecM, MeteoData &md) 
{
	if (index >= vecM.size())
		throw IOException("The index of the element to be resampled is out of bounds", AT);
	...
	// exact value or linearly interpolate, to get the correct value
	const bool processed_gap = processKnownGaps(resampling_date, paramindex, position, vecM, md);
	if (processed_gap) return true;
	...
	if (new_gap.isGap()) {
		...
		resampleInterpolationData(length_gap_interpol, endIdx_interpol, startIdx_interpol, new_gap, data_start_date, data_end_date, data_vec_before, data_vec_after, has_data_before, has_data_after, paramindex, data, dates, length);
