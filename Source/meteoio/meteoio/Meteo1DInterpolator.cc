...
static bool checkDeprecatedWindowSize(const Config &cfg) {
    if (cfg.keyExistsRegex(".*WINDOW_SIZE.*", Meteo1DInterpolator::interpol_section)) {
        std::cout << "WARNING: Using deprecated key: WINDOW_SIZE in section " + Meteo1DInterpolator::interpol_section + ".\n Please use MAX_GAP_SIZE instead. (WINDOW_SIZE will only be supported for the following 2 releases)\n";
        return true;
    }
    return false;
}
...
Meteo1DInterpolator::Meteo1DInterpolator(const Config &in_cfg, const char &rank, const IOUtils::OperationMode &mode)
    ...
    if (checkDeprecatedWindowSize(cfg)) {
        gap_size_key = "WINDOW_SIZE";
    }
...
static int findIndex(const std::string& key) {
    ...
    std::regex pattern("^.+::resample(\\d+)$", std::regex::icase);
...
void Meteo1DInterpolator::addAlgorithmToStack(const std::string& parname,const std::string& algo_name, const std::vector<std::pair<std::string, std::string>>& vecArgs, const double& i_max_gap_size) {
    std::shared_ptr<ResamplingAlgorithms> algo_ptr( ResamplingAlgorithmsFactory::getAlgorithm(algo_name, parname, i_max_gap_size, vecArgs, cfg) );
...
void Meteo1DInterpolator::processAlgorithms(const std::string &parname, const std::vector<std::pair<int, std::string>> &vecAlgos, std::string base_parname, const IOUtils::OperationMode &mode,
                                            const char &rank) {
    ...
    if (base_parname.empty()) { // in case we dont even care about the height
