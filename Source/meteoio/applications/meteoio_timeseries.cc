...
static void validMeteoData(const std::vector<std::string>& enforce_variables, const mio::MeteoData& md, const mio::DataQA& data_qa)
{
	...
	for (const std::string& var : enforce_variables) {
		if (md(var) == mio::IOUtils::nodata) {
			data_qa.printQA(mio::DataQA::MISSING, stat, var, "", md.date);
		}
	}
}
...
static void generatePointTimeseries(const Config& cfg, const Date& dateBegin, const Date& dateEnd, const bool& showProgress)
{
	...
	if (data_qa.isEnabled()) cfg.getValue("QA_CHECK_MISSING", "General", enforce_variables, IOUtils::nothrow);
	...
	for (Date d=dateBegin; d<=dateEnd; d+=samplingRate) { //time loop
		...
		for (const MeteoData& md : Meteo) {
			if (data_qa.isEnabled()) validMeteoData( enforce_variables, md, data_qa ); //check that we have everything we need
