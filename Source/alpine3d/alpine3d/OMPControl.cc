...
namespace OMPControl
{
	...
	{
		if (nbworkers > dimx) {
			throw mio::IOException("[E] More OpenMP workers (" + std::to_string(nbworkers) + ") than dimx (" + std::to_string(dimx) + "). Cannot generate slices.", AT);
		}
		if (nbworkers < dimx) {
	                              ...
	                              const mio::Grid2DObject& landuse, std::vector<std::vector<size_t> >& omp_snow_stations_ind)
	{
		...
		for (size_t ix = 0; ix < dimx; ix++) {
			for (size_t iy = 0; iy < dimy; iy++) {
				if (SnowpackInterfaceWorker::skipThisCell(landuse(ix,iy), dem(ix,iy))) n_skip_cell++;
			}
		}
		...
		for(size_t i=0;i<snow_station.size();++i) {
			...
			if(snow_station.at(i)!=NULL) worker_n++;
