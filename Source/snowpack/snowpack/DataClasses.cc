...
SnowStation::SnowStation(const bool& i_useCanopyModel, const bool& i_useSoilLayers, const bool& i_isAlpine3D,
...
#endif
	meta(), cos_sl(1.), sector(0), Cdata(nullptr), Seaice(nullptr), pAlbedo(0.), Albedo(0.),
	...
	ReSolver_dt(-1),
#ifndef SNOWPACK_CORE
	windward(false), TimeCountDeltaHS(0.), allow_inflate(i_allow_inflate)
#endif
        useCanopyModel(i_useCanopyModel), 
...
#else
SnowStation::SnowStation(const SnowpackConfig& cfg, const bool& i_isAlpine3D) : meta(), cos_sl(1.), sector(0), Cdata(), Seaice(nullptr), pAlbedo(0.), Albedo(0.),
#endif
	SoilAlb(0.), SoilEmissivity(0.), BareSoil_z0(0.), SoilNode(0), Ground(0.),
	cH(0.), mH(IOUtils::nodata), mass_sum(0.), swe(0.), lwc_sum(0.), lwc_sum_soil(0.), swc_sum_soil(0), hn(0.), rho_hn(0.),
#ifndef SNOWPACK_CORE
	...
	ReSolver_dt(-1),
#ifndef SNOWPACK_CORE
	windward(false), TimeCountDeltaHS(0.), allow_inflate(cfg.get("INFLATE_ALLOW", "Snowpack", isOperational)),
#endif
...
SnowStation::SnowStation(const SnowStation& c) :
	...
	ReSolver_dt(-1),
#ifndef SNOWPACK_CORE
	windward(c.windward), TimeCountDeltaHS(c.TimeCountDeltaHS), allow_inflate(c.allow_inflate)
#endif
	useCanopyModel(c.useCanopyModel),
...
#else
bool SnowStation::needDeflateInflate(const double&, const double&) {return false;}
#endif
...
#else
inline void deflateInflate(const CurrentMeteo&, double&, double&, const bool&) {}
#endif
                            ...
                            double& tot_mass_in)
{
	...
	if (lw_net != mio::IOUtils::nodata && tss != mio::IOUtils::nodata) {
		...
		prn_msg(__FILE__, __LINE__, "msg+", date, "FIXME!");
