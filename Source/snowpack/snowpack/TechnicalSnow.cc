 ...
 * @author Pirmin Ebner and others
...
/**
 * @brief Defined time when the slope preparation happens (9:00PM) and only for the winter season (week < 17 && week > 46).
...
bool TechSnow::prepare(const mio::Date& current_date)
{
	...
	if (hour==21 && minute==00) return true;
...
/**
 * @brief Perform technical snow preparation. The technical snow preparation has only an influence on the upper 40 cm. The maximum preparation density is 450 kg/m3.
