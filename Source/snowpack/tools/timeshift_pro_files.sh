...
s=$(tac $1 | grep -m1 ^0500 | awk -F, '{os=(substr($2,7,1)=="-")?(1):(0); printf("%04d-%02d-%02dT%02d:%02d:%02d", substr($2,7,4+os), substr($2,4+os,2), substr($2,1+os,2), substr($2,12+os,2), substr($2,15+os,2), substr($2,19+os,2))}')
awk -F, -v t=$2 -v s=${s} 'BEGIN {minunixtime=-210866760000; os=(substr(t,1,1)=="-")?(1):(0); td=mktime(sprintf("%04d %02d %02d %02d %02d %02d 0", substr(t,1,4+os), substr(t,6+os,2), substr(t,9+os,2), substr(t,12+os,2), substr(t,15+os,2), substr(t,19+os,2))); sd=mktime(sprintf("%04d %02d %02d %02d %02d %02d 0", substr(s,1,4+os), substr(s,6+os,2), substr(s,9+os,2), substr(s,12+os,2), substr(s,15+os,2), substr(s,19+os,2)))} {if(!data || substr($1,1,4) != "0500") {print} else {os=(substr($2,7,1)=="-")?(1):(0); fd=mktime(sprintf("%04d %02d %02d %02d %02d %02d, 0", substr($2,7,4+os), substr($2,4+os,2), substr($2,1+os,2), substr($2,12+os,2), substr($2,15+os,2), substr($2,19+os,2))); nd=(fd-(sd-td)); if(nd<minunixtime) {nd=minunixtime}; printf "0500,%s\n", strftime("%d.%m.%Y %H:%M:%S", nd)}; if(/DATA/) {data=1}}' $1