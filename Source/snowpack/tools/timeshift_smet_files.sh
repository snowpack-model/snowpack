...
s=$(tail -1 ~/avx/output/Laguna_Angela.smet | awk '{print $1}')
awk -v t=$2 -v s=${s} 'BEGIN {minunixtime=-210866760000; os=(substr(t,1,1)=="-")?(1):(0); td=mktime(sprintf("%04d %02d %02d %02d %02d %02d 0", substr(t,1,4+os), substr(t,6+os,2), substr(t,9+os,2), substr(t,12+os,2), substr(t,15+os,2), substr(t,19+os,2))); sd=mktime(sprintf("%04d %02d %02d %02d %02d %02d 0", substr(s,1,4+os), substr(s,6+os,2), substr(s,9+os,2), substr(s,12+os,2), substr(s,15+os,2), substr(s,19+os,2)))} {if(!data) {print} else {os=(substr($1,1,1)=="-")?(1):(0); fd=mktime(sprintf("%04d %02d %02d %02d %02d %02d, 0", substr($1,1,4+os), substr($1,6+os,2), substr($1,9+os,2), substr($1,12+os,2), substr($1,15+os,2), substr($1,19+os,2))); nd=(fd-(sd-td)); if(nd<minunixtime) {nd=minunixtime}; printf "%s", strftime("%Y-%m-%dT%H:%M:%S", nd); for(i=2; i<=NF; i++) {printf " %s", $i}; printf "\n"}; if(/\[DATA\]/) {data=1}}' $1