...
awk '{n++; if(NR==1) {if(substr($1,1,5)=="ERROR") print "ERROR"}; if((NR==3 && $NF*$NF>'${testvalue_sum}'*'${testvalue_sum}') || (NR==4 && $NF*$NF>'${testvalue_abssum}'*'${testvalue_abssum}')) {error=1}; textline[n]=$0} END {if(error==1) {print "ERROR"}; for(i=1; i<=n; i++) {print textline[i]}}'
