...
double SnLaws::parameterizedSnowAlbedo(const std::string& i_snow_albedo, const std::string& i_albedo_parameterization, const std::string& i_albAverageSchmucki,
                                       const double& i_albedo_fixedValue, const ElementData& Edata, const double& Tss, const CurrentMeteo& Mdata, const SnowStation& Xdata)
{
	...
	else if (i_albedo_parameterization == "RACMO") {
		// Albedo parametrization for Kuipers albedo
...
// 			double x = MIN(pow(Mdata.odc/3./u,.5), 1.);
...
// 			double dh = 0;
// 			if (Mdata.p!=0)
// 				dh = MIN(0.03247*log(Mdata.p/153880.0),0.0);
		...
		//double ppm = 0.1;
		//double AA = MIN(1,2.1*pow(zmwe_i[0],1.35*(1-as_top)-0.1*ppm-0.13));
...
double SnLaws::snowViscosityTemperatureTerm(const double& Te)
{
	...
	case t_term_arrhenius_critical: {
		const double Q_fac = 0.24; // 0.39; // Adjust Q to snow; from Schweizer et al. (2004): 0.24
		const double criticalExp = 0.3; // 0.7; //0.5; //0.3; //
